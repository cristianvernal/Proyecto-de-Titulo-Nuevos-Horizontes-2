...
export const getAsignGrades = (
  ...
  return async (dispatch) => {
    ...
    try {
      ...
      dispatch({
        type: types.ASIGNGRADES_GET_SUCCESS,
        payload: {
          users: AsignGradeList,
      ...
      console.log(error);
      dispatch({
        type: types.ASIGNGRADES_GET_FAILURE,
        payload: error,
      });
    }
  };
...
export const getMoreAsignGrades = (
  limit: number = types.TABLE_LIMIT_DEFAULT
): AppThunk => {
  return async (dispatch, getState) => {
    ...
    const { asignGrades, totalDocs, lastDoc } = getState().asignGradeReducer;
    try {
      const response = await firestore
        .collection("Notas")
      ...
      const AsignGradeList = response.docs.map((x) => ({
        ...x.data(),
        id: x.id,
      }));
      ...
      dispatch({
        type: types.ASIGNGRADES_GET_SUCCESS,
        asignGrades: asignGrades.concat(AsignGradeList as AsignGrade[]),
        totalDocs: totalDocs,
        lastDoc: response.docs[response.docs.length - 1],
      });
    } catch (error: any) {
...
export function AddAsignGrade(data: Partial<AsignGrade>): AppThunk {
    return async (dispatch) => {
      dispatch({
        type: types.ASIGNGRADES_ADD_SUBMITING,
      });
      try {
        ...
        dispatch(getAsignGrades());
        dispatch({
          type: types.ASIGNGRADES_ADD_SUCCESS
        })
      } catch (error: any) {
        console.log(error);
        dispatch({
          type: types.ASIGNGRADES_ADD_FAILURE,
  ...
  export const setAddEmployeeInital = (): Action => ({
    type: types.ASIGNGRADES_ADD_INITIAL,
  });
